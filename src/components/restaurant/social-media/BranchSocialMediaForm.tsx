...
export default function BranchSocialMediaForm({ updatedData,allBranches, onSuccess }: any) {
  const videoRef = useRef<HTMLInputElement>(null);
  const imageRef = useRef<HTMLInputElement>(null);
  const titleRef = useRef<HTMLInputElement>(null);
  const descriptionRef = useRef<HTMLInputElement>(null);
  const [branchId, setBranchId] = useState<string>(updatedData?.branch_id || "");
  const [isSubmitting, setIsSubmitting] = useState(false);

    useEffect(() => {
    if (updatedData) {
      setBranchId(updatedData.branch_id || "");
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Upload Restaurant Branch Social Media</h1>
