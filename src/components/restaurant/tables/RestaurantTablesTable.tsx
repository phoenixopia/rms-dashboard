...
import RestaurantTableFilters from "./RestaurantTableFilters";
...
export default function RestaurantTablesTable({ data }: any) {
  ...
  {addFilter && (
    <div className="p-5 border rounded-xl bg-muted/40 shadow-sm animate-in fade-in-50">
      <RestaurantTableFilters branches={branches} />
    </div>
  )}
</div>
            ...
            <TableBody>
              {data?.map((user: any, index: any) => {
                return (
                  ...
                  >
                    <TableCell>{user?.table_number}</TableCell>
                    <TableCell>{user?.Branch?.name ?? "N/A"}</TableCell>
                    <TableCell>{user?.capacity ?? "N/A"}</TableCell>
                    <TableCell>
                      <span
