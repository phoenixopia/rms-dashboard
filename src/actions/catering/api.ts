...
export async function updateMenu(id: string, data: any) {
  ...
  try {
    const response = await fetch(`${BASEURL}/menu/update-menu/${id}`, {
      method: 'PUT',
      headers: {
        "Content-Type": "application/json",
          "Authorization": `Bearer ${authToken}`
...
export const getAllMenuCatering = async (
  page: number = 1,
  limit: number = 10,
)=> {
  const url = `${BASEURL}/catering/get-all-catering?page=${page}&limit=${limit}`;
  const authToken = await getAuthToken();
...
export const getAllAcceptedCateringRequests = async (
  page: number = 1,
  limit: number = 10,
)=> {
  const url = `${BASEURL}/catering/list-catering-quotes?page=${page}&limit=${limit}`;
  const authToken = await getAuthToken();
...
export async function acceptMenuCateringRequest(formData: any,itemId:any) {
  ...
  try {
    ...
    const response = await api.put(`/catering/accept-with-quote/${itemId}`, formData, {
      headers: {
        Authorization: `Bearer ${authToken}`,
        "Content-Type": "application/json",
      },
    ...
    return {
      success: true,
      data: response.data,
      message: "Catering request successfully accepted",
    };
  } catch (err: any) {
    return {
      success: false,
...
export const rejectCateringRequests = async (id: string) => {
  ...
  try {
    const res = await fetch(`${BASEURL}/catering/reject-request/${id}`, {
      method: "PATCH",
      headers: {
      "Content-Type": "application/json",
          "Authorization": `Bearer ${authToken}`
 ...
 console.log(res,'------')
    ...
    console.error("Failed to reject catering request:", error);
    return { success: false, message: "Failed to reject catering request" };
  }
};
...
export async function createCateringImage(formData:FormData,itemId:any){
   ...
   console.log(itemId,'item id of ')
    ...
    return {
      success: true,
      data: response.data,
      message: "Catering image created successfully",
    };
  } catch (err: any) {
    return {
      success: false,
      message: err?.response?.data?.message || "Failed to create catering image",
    };
  }

}

export async function createCatering(data: any) {

  try {
    const authToken = await getAuthToken();
    
    const response = await api.post("/catering/create-catering",data, {
      headers: { Authorization: `Bearer ${authToken}` },
    });


    return { success: true, message: "Catering created successfully!" };
  } catch (err: any) {
    return {
      success: false,
      message: err?.response?.data?.message || "Failed to create catering.",
    };
  }
}
...
export async function updateMenuCatering(formData: any,itemId:any) {
  ...
  try {
    ...
    const response = await api.put(`/catering/update-basic-info/${itemId}`, formData, {
      headers: {
        Authorization: `Bearer ${authToken}`,
        "Content-Type": "application/json",
      },
