...
import { getAllAcceptedCateringRequests, getAllMenuCatering, getAllMenuCateringRequests } from "@/actions/catering/api";
import CateringRequestsTable from "@/components/restaurant/catering-requests/CateringRequestsTable";
import AcceptedCateringRequestsTable from "@/components/restaurant/accepted-catering-requests/AcceptedCateringRequestsTable";
export default async function CateringPage({
  ...
  try {
    data = await getAllAcceptedCateringRequests(page, limit);
  } catch (err: any) {
    console.error("Error fetching accepted catering request data:", err);
    error = `Failed to load accepted catering requests. Please try again later.${err}`;
  }
  ...
  if(!data || data?.data?.data?.length === 0){
    return(
      ...
      <h1 className="mb-4 text-2xl font-bold">Accepted Catering Requests</h1>
  ...
  console.log(data,'accepted catering request data')
  ...
  return (
    <div className="container mx-auto py-5">
      ...
      ) : <AcceptedCateringRequestsTable data={data?.data?.data} />}
